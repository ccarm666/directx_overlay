...
void Game::Draw(float gTime)
{
	...
	for (i = 1; i < MAX_NUMBER_OF_BUTTONS + 1; i++) {
		...
		if (tobii_button_matrix[i].button_face->drawIt && tobii_button_matrix[i].valid_index) tobii_button_matrix[i].button_face->Draw(gTime);
		if ((gameTime->totalGameTime - tobii_button_matrix[i].number_first_on) < tobii_button_matrix[i].number_timer) {
...
//		Init_tobii_button_matrix();
	...
	else if (!strncmp(result, "Change_background_texture", 100))
	{
		...
		udp_debug_print(result);
		udp_debug_print("********");
		change_background_texture("C:\\Users\\steve\\Desktop\\SpriteWithTimer\\mycomm.png");
