...
//		Init_tobii_button_matrix();
	...
	else if (!strncmp(result, "Change_background_texture", 100))
	{
		...
		udp_debug_print(result);
		udp_debug_print("********");
		std::string s = "Change_background_texture " + std::to_string(xsize) + " " + std::to_string(ysize);
//		MessageBox(NULL, s.c_str(), NULL, NULL);
		change_background_texture("C:\\Users\\steve\\Desktop\\SpriteWithTimer\\mycomm.png", xsize,
			ysize);
//		exit(0);
		if (debug)
		{
...
void Game::change_background_texture(char* name,int xsize,int ysize) {
	//if (tobii_menu) { delete tobii_menu; tobii_menu = 0; }
	//tobii_menu = new GameplayObject(0, 0, 0.0f, 0.0f, 0.0f);
	//delete backGround;
