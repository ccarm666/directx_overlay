...
#system 1, "C:\\Users\\steve\\Desktop\\SpriteWithTimer\\Release\\SpriteWithTimer.exe .\\tobii_weights_matrix_pic.bmp $globe_x $globe_y";
...
#&signal_handler;
$parm_file = $parm_dir . "communicator_36.prm";
#my($globe_x, $globe_y) = imgsize("tobii_weights_matrix_pic.bmp");
print "x is $globe_x, y is $globe_y";
#send_msg_to_netv("Change_background_texture,.\\tobii_weights_matrix_pic.bmp,$globe_x,$globe_y"); 
send_msg_to_netv("Change_background_texture,.\\tobii_weights_matrix_pic.bmp"); 
sleep 1;
&wait_move_window("SpriteUpdateWithTimer",250,50);
for ($i=0;$i<36;$i++) {
  my $row=int($i/6);
  my $col=($i % 6);
...
while(1) {
	...
	&make_windows_top_by_class("SpriteUpdateWithTimer");
	  ...
	  &wait_move_window("TForm",-1450,0);	
   	  &wait_move_window("TDisplayForm",250,10);
