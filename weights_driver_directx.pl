...
my($globe_x, $globe_y) = imgsize("tobii_weights_matrix_pic.bmp");
#system 1, 'C:\Users\steve\Desktop\SpriteWithTimer\Release\SpriteWithTimer.exe',  'C:\Users\steve\Desktop\SpriteWithTimer\tobii_weights_matrix_pic.bmp',$globe_x ,$globe_y;
system 1, '.\Release\SpriteWithTimer.exe',  'tobii_weights_matrix_pic.bmp',$globe_x ,$globe_y;
#exit(0);
my $matrix_xoff=55;
my $matrix_yoff=50;
my $xdelta=128;
...
sleep 1;
my $i;
$parm_file = $parm_dir . "weights_directx_36.prm";
#my($globe_x, $globe_y) = imgsize("tobii_weights_matrix_pic.bmp");
print "x is $globe_x, y is $globe_y";
send_msg_to_netv("Change_background_texture,.\\tobii_weights_matrix_pic.bmp,$globe_x,$globe_y"); 
sleep 1;
&wait_move_window("SpriteUpdateWithTimer",250,80);
for ($i=0;$i<36;$i++) {
  my $row=int($i/6);
  my $col=($i % 6);
  send_alter_msg($i+1,$col*$xdelta+$matrix_xoff,$row*$ydelta+$matrix_yoff);
#print "$row $col \n";
}
start_bci2000($parm_file);
#sleep (2);
&wait_move_window("TDisplayForm",1400,10);
while(1) {
	&make_windows_top_by_class("SpriteUpdateWithTimer");	
	&wait_move_window(OPER,-1400,0); # move bci oper windows off of main window
	&wait_move_window("TVisGraphForm",-1400,0);
	&wait_move_window("TVisForm",-1400,0);
...
sub find_process {
    ...
    foreach my $process (@processinfo) {
        write_to_log $process->{'Name'};
        if ( index( $process->{'Name'}, $proc ) != -1 ) { return 1 }
    }
    return (0);
}
sub find_process_by_name {
    ...
    foreach my $process (@processinfo) {
        write_to_log $process->{'Name'};
        if ( index( $process->{'Name'}, $proc ) != -1 ) { return $process->{'ProcessId'} }
    }
    return (0);
}
...
sub find_firefox_win {
 ...
 sub send_alter_msg {
  my($bid,$left,$top)=@_;
  my $nmsg = "Alter_coordxy,$bid,$left,$top";
  send( SOCKET, $nmsg, 0, $portaddr ) == length($nmsg)
    or die "cannot send to $netv_ip($netv_port): $!";
}
