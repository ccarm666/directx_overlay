#dependencies nirosft MultiMonitorTool.exe in default directory nircmd.exe in c:\\WINDOWS\\ for screenshot
use strict;
use warnings;
use Win32;
use Win32::OLE;
...
my($primaryx,$primaryy)=&get_displays_info();
...
#		$num_mons++;
#        print "Monitor resolution $num_mons : ",
...
#	print "Im in get_closest_parm with  $number_of_buttons and $area\n";
    while ( $file = readdir(DIR) ) {
#	     print " looking at $file\n";
        if ( $file =~ /^communicator_directx_\d+.prm/ ) {
#		   print " looking at $file\n";
#            write_to_log "$file\n";
            my ($num) = $file =~ /^communicator_directx_(\d+)\.prm/;
#            write_to_log "$num\n";
#             print "in get_closest_parm num is $num\n";
    ...
    if ($found) {
        ...
        print "could not find approriate parm file for $number_of_buttons\n";
		&signal_handler();
    }
}
...
#	  SetActiveWindow($cap);
 #    `c:\\WINDOWS\\nircmd.exe  savescreenshotwin "C:\\Users\\steve\\Desktop\\SpriteWithTimer\\mycomm.png"`;
       `c:\\WINDOWS\\nircmd.exe  savescreenshotwin "mycomm.png"`;
...
sub find_firefox_win {
 ...
 my $bmin;
 my $distance = 1000000000;
 my $smallest = 1000000000;
 my $area;
 $bmin=-1;
 my $tdistance;
 my $x1;
 my $sel_button=0;
 my ($left_b,$right_b,$top_b,$bottom_b,$cur_distance,$centerx,$centery);
 foreach $key (keys %tobii_buttons) {
   $left_b =$tobii_buttons{$key}{"left"};
   $right_b = $tobii_buttons{$key}{"right"};
   $top_b = $tobii_buttons{$key}{"top"};
   $bottom_b = $tobii_buttons{$key}{"bottom"};
   $centerx = ($right_b+$left_b)/2;
   $centery = ($bottom_b+$top_b)/2;  
   if ((($x > $tobii_buttons{$key}{"left"}) && ($x < $tobii_buttons{$key}{"right"})) && ($y < $tobii_buttons{$key}{"bottom"}) && 
         ($y > $tobii_buttons{$key}{"top"})) {
     $cur_distance=(($centerx-$x)*($centerx-$x))+(($centery-$y)*($centery-$y));
	 if ($cur_distance < $smallest) {
	    $smallest = $cur_distance;
		$sel_button = $key;
		print "possible key is $key "," left=",$tobii_buttons{$key}{"left"}, " right=",$tobii_buttons{$key}{"right"}," bottom=",
	       $tobii_buttons{$key}{"bottom"}," top=",$tobii_buttons{$key}{"top"},"\n"; 
	 }
 ...
 sub get_displays_info {
   `c:\\WINDOWS\\MultiMonitorTool.exe /scomma "display.csv"`;
   my $filename = 'display.csv';   
   ...
   while ( my $line = <$fh> ) {
     ...
     if ($numlines != 1) {
       print $line;
#                                                   	   1600 X 900,"1920, 0","3520, 900",  
	   my($xsize,$ysize,$left,$top,$right,$bottom)=$line=~/(\d+)\s+X\s+(\d+)\,\"(-?\d+)\,\s+(-?\d+)\"\,\"(-?\d+)\,\s+(-?\d+)/;
	   print "xsize=$xsize ysize=$ysize left=$left top=$top right=$right bottom=$bottom\n";
	   if(($top==0) && ($left==0)) {
	     close $fh; 
#		 exit(0);
         print "about to return monitor size $xsize,$ysize\n";
	     return($xsize,$ysize);
       }
	 }
   }
